...
namespace {
    ...
    void check_length_iter ( const Container &sequence, std::size_t expected_length, Compare cmp )
    {
        std::size_t size = ba::longest_increasing_substring_length(sequence.begin(), sequence.end(), cmp);
        BOOST_CHECK_EQUAL(size, expected_length);
    }

    template<typename Container, typename Compare>
    void check_length_range ( const Container &sequence, std::size_t expected_length, Compare cmp )
    {
        typedef typename Container::const_iterator const_iterator;
        boost::iterator_range<const_iterator> range(sequence.begin(), sequence.end());
        std::size_t size = ba::longest_increasing_substring_length(range, cmp);
        BOOST_CHECK_EQUAL(size, expected_length);
    }

    template<typename Container, typename Compare>
    void check_length_pointer ( const Container &sequence, std::size_t expected_length, Compare cmp )
    {
        ...
        std::size_t size = ba::longest_increasing_substring_length(sBeg, sEnd, cmp);
        BOOST_CHECK_EQUAL(size, expected_length);
    }

    template<typename Container, typename Compare>
    void check_length_object ( const Container &sequence, std::size_t expected_length, Compare cmp )
    {
        typedef typename Container::const_iterator const_iterator;

        ba::longest_increasing_substring<const_iterator, Compare> ls(cmp);
        std::size_t resSize = ls.compute_length(sequence.begin(), sequence.end());
        BOOST_CHECK_EQUAL(resSize, expected_length);
    }

    // Result content checking
    template<typename Container, typename Compare>
    void check_lis_iter ( const Container &sequence, const Container &expected_lis, Compare cmp )
    {
        typedef typename Container::const_iterator const_iterator;
        typedef typename ba::longest_increasing_substring<const_iterator, Compare>::result_type result_type;

        result_type res = ba::longest_increasing_substring_search(sequence.begin(), sequence.end(), cmp);
        BOOST_CHECK_EQUAL_COLLECTIONS(res.first, res.second, expected_lis.begin(), expected_lis.end());
    }

    template<typename Container, typename Compare>
    void check_lis_range ( const Container &sequence, const Container &expected_lis, Compare cmp )
    {
        typedef typename Container::const_iterator const_iterator;
        typedef boost::iterator_range<const_iterator> range_type;
        range_type range(sequence.begin(), sequence.end());

        range_type resRange1 = ba::longest_increasing_substring_search(range, cmp);
        BOOST_CHECK_EQUAL_COLLECTIONS(resRange1.begin(), resRange1.end(), expected_lis.begin(), expected_lis.end());

        ba::longest_increasing_substring<const_iterator, Compare> ls2 = ba::make_longest_increasing_substring(range, cmp);
        range_type resRange2 = ls2(range);
        BOOST_CHECK_EQUAL_COLLECTIONS(resRange2.begin(), resRange2.end(), expected_lis.begin(), expected_lis.end());
    }

    template<typename Container, typename Compare>
    void check_lis_pointer ( const Container &sequence, const Container &expected_lis, Compare cmp )
    {
        typedef const typename Container::value_type *ptr_type;
        typedef typename ba::longest_increasing_substring<ptr_type, Compare>::result_type result_type;

        ptr_type sBeg = sequence.size () == 0 ? NULL : &*sequence.begin ();
        ptr_type sEnd = sBeg + sequence.size ();
        result_type res = ba::longest_increasing_substring_search(sBeg, sEnd, cmp);
        BOOST_CHECK_EQUAL_COLLECTIONS(res.first, res.second, expected_lis.begin(), expected_lis.end());
    }

    template<typename Container, typename Compare>
    void check_lis_object ( const Container &sequence, const Container &expected_lis, Compare cmp ) {
        typedef typename Container::const_iterator const_iterator;
        typedef typename ba::longest_increasing_substring<const_iterator, Compare>::result_type result_type;
    ...
    template<typename Container, typename Compare>
    void check_lis ( const Container &sequence, const std::vector<Container> &expected_lises, Compare cmp ) {
        typedef typename Container::const_iterator const_iterator;
        typedef typename ba::longest_increasing_substring<const_iterator, Compare>::result_type result_type;
        ...
        for (std::size_t i = 0; i < expected_lises.size(); ++i) {
            ...
            any_equal |= std::equal(res.first, res.second, expected_lis.begin()/*, expected_lis.end()*/);
        }
    ...
    template<typename Container, typename Compare>
    void check_one_iter ( const std::string& test_name, const Container &sequence, std::size_t expected_length, const Container &expected_lis, Compare cmp )
    {
        std::cout << "(Iterators) Sequence " << test_name << " is " << sequence.size() << " elements long; " << std::endl;
    ...
    // Check using ranges
    template<typename Container, typename Compare>
    void check_one_range ( const std::string& test_name, const Container &sequence, std::size_t expected_length, const Container &expected_lis, Compare cmp )
    {
        std::cout << "(Ranges) Sequence " << test_name << " is " << sequence.size() << " elements long; " << std::endl;
    ...
    template<typename Container, typename Compare>
    void check_one_pointer ( const std::string& test_name, const Container &sequence, std::size_t expected_length, const Container &expected_lis, Compare cmp )
    {
        std::cout << "(Pointers) Sequence " << test_name << " is " << sequence.size() << " elements long; " << std::endl;
    ...
    template<typename Container, typename Compare>
    void check_one_object ( const std::string& test_name, const Container &sequence, std::size_t expected_length, const Container &expected_lis, Compare cmp )
    {
        std::cout << "(Objects) Sequence " << test_name << " is " << sequence.size() << " elements long; " << std::endl;
    ...
    void check_one ( const std::string& test_name, const Container &sequence, std::size_t expected_length, const Container &expected_lis, Compare cmp )
    {
        ...
        check_one_pointer ( test_name, sequence, expected_length, expected_lis, cmp );
        check_one_object ( test_name, sequence, expected_length, expected_lis, cmp );
    }
    ...
    template<typename Container, typename Compare>
    void check_one ( const std::string& test_name, const Container &sequence, std::size_t expected_length, const std::vector<Container> &expected_lises, Compare cmp )
    {
        check_length_iter ( sequence, expected_length, cmp );
    ...
    template<typename Container>
    void check_one ( const std::string& test_name, const Container &sequence, std::size_t expected_length, const std::vector<Container> &expected_lises )
    {
        typedef typename Container::value_type value_type;
...
BOOST_AUTO_TEST_CASE( test_main )
{
    ...
    vec seq6 = list_of(0) (5) (8) (4) (12) (2) (10) (6) (14) (1) (9) (5) (13) (3) (11) (7) (15) (14) (13);
