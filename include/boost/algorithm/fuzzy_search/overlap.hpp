...
#ifndef BOOST_ALGORITHM_OVERLAP_HPP
#define BOOST_ALGORITHM_OVERLAP_HPP
...
VarType overlap_coefficient(RAIterator1 begin1, RAIterator1 end1,
                         RAIterator2 begin2, RAIterator2 end2)
{
    ...
    return  static_cast<double>(inter.size()) / static_cast<double>(std::min(vec1.size(), vec2.size()));
}
...
{
    return overlap_coefficient<VarType>(boost::begin(range1), boost::end(range1),
                                     boost::begin(range2), boost::end(range2));
};
}}

#endif //BOOST_ALGORITHM_OVERLAP_HPP
