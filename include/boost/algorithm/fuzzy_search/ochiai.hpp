...
#ifndef BOOST_ALGORITHM_OCHIAI_HPP
#define BOOST_ALGORITHM_OCHIAI_HPP
...
VarType ochiai_coefficient(RAIterator1 begin1, RAIterator1 end1,
                                       RAIterator2 begin2, RAIterator2 end2)
{
    ...
    return  static_cast<double>(intersect.size()) / sqrt(static_cast<double>(vec1.size() * vec2.size()));
}
...
{
    return ochiai_coefficient<VarType>(boost::begin(range1), boost::end(range1),
                                                   boost::begin(range2), boost::end(range2));
};
}}

#endif //BOOST_ALGORITHM_OCHIAI_HPP
